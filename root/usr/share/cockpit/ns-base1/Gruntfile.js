module.exports = function (grunt) {
  ...
  grunt.registerTask('rsync', 'Sync folder with remote host', function (login, port, dest) {
    ...
    grunt.task.run([
      ['shell:rsync', login, port, 'dist/', dest].join(':'), ['shell:rsync', login, port, 'test/', dest].join(':'), ['shell:rsync', login, port, 'node_modules/mocha/mocha.*', dest].join(':'), ['shell:rsync', login, port, 'node_modules/should/should.js', dest].join(':'),
    ]);
  });
};