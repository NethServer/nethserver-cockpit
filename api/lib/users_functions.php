...
function user_membership($user) {
    $output = shell_exec(escapeshellcmd("/usr/libexec/nethserver/list-user-membership -s ".escapeshellarg($user)));
    return json_decode($output,TRUE);
}

/* Return the list of users inside a group */
function group_members($group) {
    $output = shell_exec(escapeshellcmd("/usr/libexec/nethserver/list-group-members -s ".escapeshellarg($group)));
    return json_decode($output,TRUE);
}

/* Return user information */
function user_info($user) {
    $output = shell_exec(escapeshellcmd("/usr/libexec/nethserver/list-users -s ".escapeshellarg($user)));
...
function cleanup_group($group, $user) {
    ...
    if (in_array($user, $members)) {
        ...
        passthru(escapeshellcmd($cmd), $tmp_ret);
        $ret += $tmp_ret;
    }
